...
public class CodeQualityCheckNode {
    ...
    private static String readAndConcatenateCodeFiles(String codeDir) {
        ...
        FileUtil.walkFiles(directory, file -> {
            ...
            if (isCodeFile(file)) {
                // 获取文件相对于根目录的相对路径。
                //FileUtil.subPath(父路径, 子路径) 是 Hutool 工具库中的方法，它会从“子路径”中去掉“父路径”部分，返回剩下的相对路径字符串。
                //例如：
                //父路径：/home/project
                //子路径：/home/project/src/main.js
                //返回值：src/main.js
                ...
                String fileContent = FileUtil.readUtf8String(file);//使用 UTF-8 编码读取整个文件的内容，返回一个字符串。
    ...
    private static boolean isCodeFile(File file) {
        String fileName = file.getName().toLowerCase();//将文件名转为小写
        //判断是否以预定义的后缀列表中的某一项结尾。
